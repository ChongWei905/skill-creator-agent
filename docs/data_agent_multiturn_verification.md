...
Use the standalone CLI to validate the real Ferry `DataAgent` path instead of calling `SkillCreatorAgent` directly.

## Run

```bash
cd /Users/weichong/Documents/new_working_area/skill-creator-agent
uv run skill-creator-chat
```
...
- `/help`: print command help inside the REPL
- `/quit`: exit
